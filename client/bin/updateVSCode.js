...
if (!match && match[1]) {
	throw new Error(
		`Unable to find 'const REQUIRED_VSCODE_VERSION' in main.ts`,
	);
}
if (match[1] !== engineVersion) {
	console.log(
		`Updating REQUIRED_VSCODE_VERSION in main.ts to ${engineVersion}`,
	);
	const updatedSource = mainSource.replace(
		regexp,
		`const REQUIRED_VSCODE_VERSION = '${engineVersion}';`,
	);
	fs.writeFileSync(mainPath, updatedSource);
} else {
	console.log(
		`No change needed: REQUIRED_VSCODE_VERSION in main.ts is already ${engineVersion}`,
	);
}
