...
export function createConverter(uriConverter?: URIConverter): Converter {
	...
	function asTextDocumentItem(
		textDocument: code.TextDocument,
	): proto.TextDocumentItem {
		return {
			uri: _uriConverter(textDocument.uri),
	...
	function asTextDocumentSaveReason(
		reason: code.TextDocumentSaveReason,
	): 1 | 2 | 3 {
		switch (reason) {
	...
	function asDidCreateFilesParams(
		event: code.FileCreateEvent,
	): proto.CreateFilesParams {
		return {
	...
	function asDidRenameFilesParams(
		event: code.FileRenameEvent,
	): proto.RenameFilesParams {
		return {
	...
	function asDidDeleteFilesParams(
		event: code.FileDeleteEvent,
	): proto.DeleteFilesParams {
		return {
			files: event.files.map((fileUri) => ({
	...
	function asWillCreateFilesParams(
		event: code.FileWillCreateEvent,
	): proto.CreateFilesParams {
		return {
			files: event.files.map((fileUri) => ({
				uri: _uriConverter(fileUri),
	...
	function asWillRenameFilesParams(
		event: code.FileWillRenameEvent,
	): proto.RenameFilesParams {
		return {
			files: event.files.map((file) => ({
				oldUri: _uriConverter(file.oldUri),
	...
	function asWillDeleteFilesParams(
		event: code.FileWillDeleteEvent,
	): proto.DeleteFilesParams {
		return {
			files: event.files.map((fileUri) => ({
				uri: _uriConverter(fileUri),
	...
	function asCompletionTriggerKind(
		triggerKind: code.CompletionTriggerKind,
	...
	function asSignatureHelpTriggerKind(
		triggerKind: code.SignatureHelpTriggerKind,
	...
	function asParameterInformation(
		value: code.ParameterInformation,
	...
	function asParameterInformations(
		values: code.ParameterInformation[],
	): proto.ParameterInformation[] {
		return values.map(asParameterInformation);
	}

	function asSignatureInformation(
		value: code.SignatureInformation,
	...
	function asSignatureInformations(
		values: code.SignatureInformation[],
	): proto.SignatureInformation[] {
		return values.map(asSignatureInformation);
	}

	function asSignatureHelp(
		value: code.SignatureHelp | undefined,
	...
	function asSignatureHelpParams(
		...
		return {
			...
			context: {
				...
				activeSignatureHelp: asSignatureHelp(
	...
	function asPositions(
		values: readonly code.Position[],
		token?: code.CancellationToken,
	): Promise<proto.Position[]> {
		return async.map(
			values,
			asPosition as (item: code.Position) => proto.Position,
			token,
		);
	}

	function asPositionsSync(
		values: readonly code.Position[],
	): proto.Position[] {
		return values.map(
			asPosition as (item: code.Position) => proto.Position,
	...
	function asDiagnosticSeverity(
		value: code.DiagnosticSeverity,
	): proto.DiagnosticSeverity {
		switch (value) {
	...
	function asRelatedInformation(
		item: code.DiagnosticRelatedInformation,
	...
	function asRelatedInformations(
		items: code.DiagnosticRelatedInformation[],
	): proto.DiagnosticRelatedInformation[] {
	...
	function asDiagnosticCode(
		value:
			| number
			| string
			| { value: string | number; target: code.Uri }
			| undefined
			| null,
	...
	function asDiagnostics(
		items: ReadonlyArray<code.Diagnostic>,
		token?: code.CancellationToken,
	): Promise<proto.Diagnostic[]> {
	...
	function asDocumentation(
		format: string,
		documentation: string | code.MarkdownString,
	...
	function asCompletionItemTags(
		tags: ReadonlyArray<code.CompletionItemTag> | undefined,
	): proto.CompletionItemTag[] | undefined {
		if (tags === undefined) {
			return tags;
		}
	...
	function asCompletionItem(
		...
		if (Is.string(item.label)) {
			...
			if (
				...
				(item.label.detail !== undefined ||
					item.label.description !== undefined)
			) {
				labelDetails = {
		...
		if (item.documentation) {
			if (
				!protocolItem ||
				protocolItem.documentationFormat === "$string"
			) {
	...
	function asSymbolTags(
		items: ReadonlyArray<code.SymbolTag>,
	): proto.SymbolTag[] {
	...
	function asCodeActionTriggerKind(
		kind: code.CodeActionTriggerKind,
	...
	function asCodeActionKind(
		item: code.CodeActionKind | null | undefined,
	...
	function asInlineCompletionContext(
		...
		return {
			...
			selectedCompletionInfo: asSelectedCompletionInfo(
	...
	function asInlineCompletionTriggerKind(
		kind: code.InlineCompletionTriggerKind,
	...
	function asSelectedCompletionInfo(
		info: code.SelectedCompletionInfo | null | undefined,
	...
	function asWorkspaceSymbol(
		item: code.SymbolInformation,
	...
	function asTooltip(
		value: string | code.MarkdownString,
	): string | proto.MarkupContent {
