...
> {
			...
			provideCodeActions: (document, range, context, token) => {
				...
				const _provideCodeActions: ProvideCodeActionsSignature = async (
					...
					const params: CodeActionParams = {
						...
						context:
							client.code2ProtocolConverter.asCodeActionContextSync(
								context,
							),
					};
							...
							async (item, token) => {
								return client
									.sendRequest(
										...
										client.code2ProtocolConverter.asCodeActionSync(
											item,
										),
										token,
									)
	...
	private getMetadata(
		...
		if (
			options.codeActionKinds === undefined &&
		...
		return {
			...
			documentation:
				this._client.protocol2CodeConverter.asCodeActionDocumentations(
