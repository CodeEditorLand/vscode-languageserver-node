...
import { DiagnosticCode, ProtocolDiagnostic } from "./protocolDiagnostic";
...
export interface Converter {
	...
	asInlineCompletionResult(
		value:
			...
			| ls.InlineCompletionList
			| undefined
			| null,
		token?: code.CancellationToken,
	): Promise<
		code.InlineCompletionItem[] | code.InlineCompletionList | undefined
	>;
...
export function createConverter(
	...
	function asDocumentSelector(
		selector: ls.DocumentSelector,
	): code.DocumentSelector {
		const result:
			| code.DocumentFilter
			| string
			| Array<code.DocumentFilter | string> = [];
	...
	function asRelatedInformation(
		...
		const result: code.DiagnosticRelatedInformation[] = new Array(
	...
	function asDiagnosticTag(
		tag: ls.DiagnosticTag,
	): code.DiagnosticTag | undefined {
		switch (tag) {
	...
	function asPosition(
		...
		return value
			? new code.Position(value.line, value.character)
			: undefined;
	}
	...
	function asDocumentation(
		value: string | ls.MarkupContent,
	): string | code.MarkdownString {
		if (Is.string(value)) {
			return value;
		} else {
			switch (value.kind) {
	...
	function asHover(
		hover: ls.Hover | undefined | null,
	): code.Hover | undefined;
	function asHover(
		hover: ls.Hover | undefined | null,
	): code.Hover | undefined {
		if (!hover) {
		...
		const converted = await async.map(
			...
			(item) => {
				return asCompletionItem(
					item,
					commitCharacters,
					list.applyKind?.commitCharacters,
					defaultRange,
	...
	): code.CompletionItemTag | undefined {
		switch (tag) {
	...
	function applyCommitCharacters(
		item: ls.CompletionItem,
		defaultCommitCharacters: string[] | undefined,
		applyKind: ls.ApplyKind | undefined,
	): string[] | undefined {
	...
	function asCompletionItemLabel(
		item: ls.CompletionItem,
	): code.CompletionItemLabel | string {
	...
	): [code.Range | InsertReplaceRange, string] {
	...
	function asTextEdit(
		edit: ls.TextEdit | undefined | null,
	): code.TextEdit | undefined {
		if (!edit) {
			return undefined;
		}
	...
	function asDocumentHighlight(
		item: ls.DocumentHighlight,
	...
	function asSymbolInformation(
		...
		const location: Omit<ls.Location, "range"> & { range?: ls.Range } =
			item.location;
		const result: code.SymbolInformation =
	...
	function fillTags(
		result: { tags?: ReadonlyArray<code.SymbolTag> },
			...
			await async.forEach(
				...
				(change) => {
					if (ls.CreateFile.is(change)) {
						...
						const edits: [
							code.TextEdit | code.SnippetTextEdit,
							code.WorkspaceEditEntryMetadata | undefined,
						][] = [];
	...
	): code.WorkspaceEditEntryMetadata;
	function asWorkspaceEditEntryMetadata(
	...
	function asColorInformation(
		ci: ls.ColorInformation,
	...
	async function asColorInformations(
		colorInformation: ls.ColorInformation[] | undefined | null,
		token?: code.CancellationToken,
	): Promise<code.ColorInformation[] | undefined> {
		if (!colorInformation) {
			return undefined;
		}
	...
	function asInlayHintLabelPart(
		part: ls.InlayHintLabelPart,
	): code.InlayHintLabelPart {
	...
	function asGlobPattern(
		pattern: ls.GlobPattern | null | undefined,
	): code.GlobPattern | undefined;
	function asGlobPattern(
		pattern: ls.GlobPattern | null | undefined,
	): code.GlobPattern | undefined {
	...
	function asInlineCompletionItem(
		item: ls.InlineCompletionItem,
	): code.InlineCompletionItem {
