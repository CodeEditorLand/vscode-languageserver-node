...
suite("Protocol Helper Tests", () => {
	...
	test("CodeActionContext", () => {
		const codeActionContext = CodeActionContext.create([
			Diagnostic.create(Range.create(1, 2, 8, 9), "message"),
		]);
	...
	test("WorkspaceEdit - documentChanges", () => {
		...
		let edits = (workspaceEdit.documentChanges![0] as TextDocumentEdit)
			.edits;
	...
	test("WorkspaceEdit - change annotations", () => {
		...
		change1.insert(
			Position.create(0, 1),
			"insert",
			ChangeAnnotation.create("label", true, "description"),
		);

		const workspaceEdit = workspaceChange.edit;
		const documentChanges = workspaceEdit.documentChanges!;
		ok(
			workspaceEdit.changeAnnotations !== undefined,
			"Change annotation defined",
		);
	...
	test("Relative Pattern", () => {
		if (process.platform === "win32") {
			const pattern: RelativePattern = {
				baseUri: Uri.file("C:\\folder1\\folder2").toString(),
				pattern: "**/*.txt",
			};
			ok(
				matchGlobPattern(
					pattern,
					Uri.file("c:\\folder1\\folder2\\file.txt"),
				),
			);
			ok(
				matchGlobPattern(
					pattern,
					Uri.file("c:\\folder1\\folder2\\folder3\\file.txt"),
				),
			);
			ok(
				!matchGlobPattern(
					pattern,
					Uri.file("c:\\folder1\\folder3\\file.txt"),
				),
			);
		} else {
			const pattern: RelativePattern = {
				baseUri: Uri.file("/folder1/folder2").toString(),
				pattern: "**/*.txt",
			};
			ok(
				matchGlobPattern(
					pattern,
					Uri.file("/folder1/folder2/file.txt"),
				),
			);
			ok(
				matchGlobPattern(
					pattern,
					Uri.file("/folder1/folder2/folder3/file.txt"),
				),
			);
			ok(
				!matchGlobPattern(
					pattern,
					Uri.file("/folder1/folder3/file.txt"),
				),
			);
		}
	});
