...
import {
	DiagnosticCode,
	ProtocolDiagnostic,
} from "vscode-languageclient/$test/common/protocolDiagnostic";
...
namespace InsertReplaceRange {
	export function is(
		value: vscode.Range | InsertReplaceRange | proto.Range,
...
function assertComplexCode(
	value: undefined | number | string | ComplexCode,
): asserts value is ComplexCode {
	if (
		value === undefined ||
		typeof value === "number" ||
		typeof value === "string"
...
function assertRange(
	value: vscode.Range | InsertReplaceRange | undefined | null,
): asserts value is vscode.Range {
	if (!value || InsertReplaceRange.is(value)) {
		throw new Error(
			`Expected a normal range but got an insert / replace range.`,
		);
	}
}

function assertInsertReplaceRange(
	value: vscode.Range | InsertReplaceRange | undefined | null,
): asserts value is InsertReplaceRange {
	if (!value || !InsertReplaceRange.is(value)) {
		throw new Error(
			`Expected an insert / replace range but got a normal range.`,
		);
	}
}

function assertTextEdit(
	value: proto.TextEdit | proto.InsertReplaceEdit | undefined | null,
): asserts value is proto.TextEdit {
	if (!value || proto.InsertReplaceEdit.is(value)) {
		throw new Error(`Expected a text edit but got an insert replace edit.`);
	}
}

function assertInsertReplaceEdit(
	value: proto.TextEdit | proto.InsertReplaceEdit | undefined | null,
): asserts value is proto.InsertReplaceEdit {
	if (!value || !proto.InsertReplaceEdit.is(value)) {
		throw new Error(
			`Expected an insert replace edit but got a normal text edit.`,
		);
	}
}
...
): asserts value is DiagnosticCode {
...
suite("Async Array", () => {
	...
	test("map", async () => {
		...
		let yielded = 0;
		const converted = await async.map(ranges, p2c.asRange, undefined, {
			yieldAfter: 2,
			yieldCallback: () => {
				yielded++;
			},
	...
	test("map async", async () => {
		...
		let yielded = 0;
		const converted = await async.mapAsync(
			ranges,
			(item): Promise<vscode.Range> => {
...
suite("Protocol Converter", () => {
	...
	function rangeEqual(
		actual: vscode.Range | proto.Range,
		expected: vscode.Range | proto.Range,
	): void {
	...
	function positionEqual(
		actual: vscode.Position,
		expected: proto.Position,
	): void;
	function positionEqual(
		actual: proto.Position,
		expected: vscode.Position,
	): void;
	function positionEqual(
		actual: vscode.Position | proto.Position,
		expected: vscode.Position | proto.Position,
	): void {
		strictEqual(actual.line, expected.line);
		strictEqual(actual.character, expected.character);
	}

	function completionEditEqual(
		text: string,
		range: vscode.Range | InsertReplaceRange,
		expected: proto.TextEdit | proto.InsertReplaceEdit,
	): void {
	...
	test("Diagnostic Severity", () => {
		strictEqual(
			p2c.asDiagnosticSeverity(proto.DiagnosticSeverity.Error),
			vscode.DiagnosticSeverity.Error,
		);
		strictEqual(
			p2c.asDiagnosticSeverity(proto.DiagnosticSeverity.Warning),
		...
		strictEqual(
			p2c.asDiagnosticSeverity(proto.DiagnosticSeverity.Hint),
	...
	test("Diagnostic Tag", () => {
		strictEqual(
			p2c.asDiagnosticTag(proto.DiagnosticTag.Unnecessary),
			vscode.DiagnosticTag.Unnecessary,
		);
		strictEqual(
			p2c.asDiagnosticTag(proto.DiagnosticTag.Deprecated),
	...
	test("Diagnostic", async () => {
		const start: proto.Position = { line: 1, character: 2 };
		const end: proto.Position = { line: 8, character: 9 };
		const location: proto.Location = proto.Location.create(
			"file://localhost/folder/file",
			proto.Range.create(0, 1, 2, 3),
		);
		const diagnostic: proto.Diagnostic = {
			...
			relatedInformation: [{ message: "related", location: location }],
		};
		...
		strictEqual(
			result.relatedInformation![0].location.uri.toString(),
			"file://localhost/folder/file",
		);
		strictEqual(
			result.relatedInformation![0].location.range.end.character,
			3,
		);

		ok(
			(await p2c.asDiagnostics([diagnostic])).every(
	...
	test("Text Edit Insert", () => {
		const edit: proto.TextEdit = proto.TextEdit.insert(
			{ line: 1, character: 2 },
			"insert",
		);
	...
	test("Text Edits", async () => {
		...
		ok(
			(await p2c.asTextEdits([edit])).every(
				(elem) => elem instanceof vscode.TextEdit,
			),
		);
	...
	test("Completion Item - Full", async () => {
		...
		const completionItem: proto.CompletionItem = {
			...
			data: "data",
			additionalTextEdits: [
				proto.TextEdit.insert({ line: 1, character: 2 }, "insert"),
			],
			command: command,
			commitCharacters: ["."],
			tags: [proto.CompletionItemTag.Deprecated],
		...
		ok(
			completionResult.every(
				(value) => value instanceof vscode.CompletionItem,
			),
		);
	});
	...
	test("Completion Item - Snippet String", () => {
		...
		strictEqual(
			(<vscode.SnippetString>result.insertText).value,
			"${value}",
		);
		strictEqual(result.range, undefined);
		strictEqual(result.textEdit, undefined);
	...
	test("Completion Item - Text Edit", () => {
		const completionItem: proto.CompletionItem = {
			label: "item",
			textEdit: proto.TextEdit.insert(
				{ line: 1, character: 2 },
				"insert",
			),
		};
	...
	test("Completion Item - Insert / Replace Edit", () => {
		const completionItem: proto.CompletionItem = {
			label: "item",
			textEdit: proto.InsertReplaceEdit.create(
				"text",
				proto.Range.create(0, 0, 0, 0),
				proto.Range.create(0, 0, 0, 2),
			),
		};
	...
	test("Completion Item - Text Edit Snippet String", () => {
		...
		strictEqual(result.textEdit, undefined);
		ok(
			result.insertText instanceof vscode.SnippetString &&
				result.insertText.value === "${insert}",
		);
		assertRange(result.range);
		assertTextEdit(completionItem.textEdit);
		rangeEqual(result.range, completionItem.textEdit.range);
	...
	test("Completion Item - Preserve Data", () => {
		...
		const result = c2p.asCompletionItem(
			p2c.asCompletionItem(completionItem),
		);
		strictEqual(result.data, completionItem.data);
	});
	...
	test("Completion Item - Documentation as PlainText", () => {
		...
		strictEqual(
			(result.documentation as proto.MarkupContent).kind,
			proto.MarkupKind.PlainText,
		);
	...
	test("Completion Item - Documentation as Markdown", () => {
		...
		strictEqual(
			(result.documentation as proto.MarkupContent).kind,
			proto.MarkupKind.Markdown,
		);
		strictEqual(
			(result.documentation as proto.MarkupContent).value,
			"# Header",
		);
	});
	...
	test("Completion Item - InsertTextMode.adjustIndentation", () => {
		...
		strictEqual(
			back.insertTextMode,
			proto.InsertTextMode.adjustIndentation,
		);
	});
	...
	test("Completion Item - Label Details", () => {
		...
		const label: vscode.CompletionItemLabel =
			result.label as vscode.CompletionItemLabel;
		...
		strictEqual(
			proto.CompletionItemLabelDetails.is(back.labelDetails),
			true,
		);
	...
	test("Completion Result - edit range", async () => {
		...
		strictEqual(result.items[0].label, "item");
		rangeEqual(
			result.items[0].range as vscode.Range,
			completionResult.itemDefaults?.editRange as proto.Range,
		);
	});
	...
	test("Completion Result - insert / replace range", async () => {
		const completionResult: proto.CompletionList = {
			...
			itemDefaults: {
				editRange: {
					insert: proto.Range.create(1, 1, 1, 1),
					replace: proto.Range.create(1, 2, 3, 4),
				},
			},
			items: [{ label: "item", data: "data" }],
		};
		...
		rangeEqual(
			(range as { inserting: vscode.Range }).inserting,
			(
				completionResult.itemDefaults?.editRange as {
					insert: proto.Range;
				}
			).insert,
		);
		rangeEqual(
			(range as { replacing: vscode.Range }).replacing,
			(
				completionResult.itemDefaults?.editRange as {
					replace: proto.Range;
				}
	...
	test("Completion Result - applyKind:replace - commitCharacters", async () => {
		const completionResult: proto.CompletionList = {
			...
			applyKind: {
				commitCharacters: proto.ApplyKind.Replace,
				data: proto.ApplyKind.Merge,
			},
	...
	test("Completion Result - applyKind:replace - data", async () => {
		const completionResult: proto.CompletionList = {
			...
			// Set other fields to "merge" to ensure the correct field was used.
			applyKind: {
				data: proto.ApplyKind.Replace,
				commitCharacters: proto.ApplyKind.Merge,
			},
			items: [{ label: "item", data: { "i": "i" } }],
		};
		const result = await p2c.asCompletionResult(completionResult);
	...
	test("Parameter Information", async () => {
		...
		ok(
			(await p2c.asParameterInformations([parameterInfo])).every(
				(value) => value instanceof vscode.ParameterInformation,
			),
		);
	});
	...
	test("Signature Information", async () => {
		...
		ok(
			(await p2c.asSignatureInformations([signatureInfo])).every(
				(value) => value instanceof vscode.SignatureInformation,
			),
		);
	});
	...
	test("Location", async () => {
		...
		ok(
			(await p2c.asReferences([location])).every(
				(value) => value instanceof vscode.Location,
			),
		);
	});
	...
	test("Definition", async () => {
		...
		const array = <vscode.Location[]>(
			await p2c.asDefinitionResult([location])
		);
	...
	test("Document Highlight", async () => {
		...
		ok(
			(await p2c.asDocumentHighlights([documentHighlight])).every(
				(value) => value instanceof vscode.DocumentHighlight,
			),
		);
	...
	test("Document Links", async () => {
		...
		ok(
			(await p2c.asDocumentLinks([documentLink])).every(
				(value) => value instanceof vscode.DocumentLink,
			),
		);
	...
	test("SymbolInformation", async () => {
		...
		ok(
			(await p2c.asSymbolInformations([symbolInformation])).every(
				(value) => value instanceof vscode.SymbolInformation,
			),
		);
	...
	test("WorkspaceSymbol", () => {
		...
		strictEqual(
			(back.location as { range?: proto.Range }).range,
			undefined,
		);
	});
	...
	test("Command", async () => {
		...
		ok(
			(await p2c.asCommands([command])).every(
				(elem) => !!elem.title && !!elem.tooltip && !!elem.command,
			),
		);
	...
	test("Code Lens", async () => {
		const codeLens: proto.CodeLens = proto.CodeLens.create(
			proto.Range.create(1, 2, 8, 9),
			"data",
		);
		...
		ok(
			(await p2c.asCodeLenses([codeLens])).every(
				(elem) => elem instanceof vscode.CodeLens,
			),
		);
	...
	test("WorkspaceEdit", async () => {
		...
		const change1 = workspaceChange.getTextEditChange({
			uri: uri1,
			version: 1,
		});
		change1.insert(proto.Position.create(0, 1), "insert");
		const uri2 = "file:///xyz.txt";
		const change2 = workspaceChange.getTextEditChange({
			uri: uri2,
			version: 99,
		});
	...
	test("Uri Rewrite", () => {
		const converter = protocolConverter.createConverter(
			(value: string) => {
	...
	test("InlineValues", async () => {
		const items: proto.InlineValue[] = [
			proto.InlineValueText.create(
				proto.Range.create(1, 2, 8, 9),
				"literalString",
			),
			proto.InlineValueVariableLookup.create(
				proto.Range.create(1, 2, 8, 9),
				"varName",
				false,
			),
			...
			proto.InlineValueEvaluatableExpression.create(
				proto.Range.create(1, 2, 8, 9),
				"expression",
			),
			proto.InlineValueEvaluatableExpression.create(
		...
		ok(
			result[0] instanceof vscode.InlineValueText &&
				result[0].text === "literalString",
		);
		ok(
			result[1] instanceof vscode.InlineValueVariableLookup &&
				result[1].variableName === "varName" &&
				result[1].caseSensitiveLookup === false,
		);
		ok(
			result[2] instanceof vscode.InlineValueVariableLookup &&
				result[2].variableName === undefined &&
				result[2].caseSensitiveLookup === true,
		);
		ok(
			result[3] instanceof vscode.InlineValueEvaluatableExpression &&
				result[3].expression === "expression",
		);
		ok(
			result[4] instanceof vscode.InlineValueEvaluatableExpression &&
				result[4].expression === undefined,
		);
	});

	test("InlayHint", async () => {
		const one: proto.InlayHint = proto.InlayHint.create(
			proto.Position.create(1, 1),
			"one",
			proto.InlayHintKind.Parameter,
		);
		one.data = "1";
		const two: proto.InlayHint = proto.InlayHint.create(
			proto.Position.create(2, 2),
			"two",
			proto.InlayHintKind.Type,
		);
		two.data = "2";
		const items = [one, two];
	...
	test("InlineCompletions", async () => {
		const items: proto.InlineCompletionItem[] = [
			proto.InlineCompletionItem.create(
				"insert text",
				"in",
				proto.Range.create(1, 2, 6, 7),
			),
			proto.InlineCompletionItem.create(
				"insert text",
				"in",
			...
			proto.InlineCompletionItem.create(
				proto.StringValue.createSnippet("insert text"),
				"in",
				proto.Range.create(1, 2, 6, 7),
				undefined,
			),
		...
		ok(
			result[0] instanceof vscode.InlineCompletionItem &&
				result[0].insertText === "insert text",
		);
		ok(
			result[0] instanceof vscode.InlineCompletionItem &&
				result[0].filterText === "in",
		);
...
suite("Code Converter", () => {
	...
	test("Text Edit Insert", () => {
		const insert = vscode.TextEdit.insert(
			new vscode.Position(1, 2),
			"insert",
		);
	...
	test("Text Edit Replace", () => {
		const replace = vscode.TextEdit.replace(
			new vscode.Range(
				new vscode.Position(1, 2),
				new vscode.Position(8, 9),
			),
	...
	test("Text Edit Delete", () => {
		const del = vscode.TextEdit.delete(
			new vscode.Range(
				new vscode.Position(1, 2),
				new vscode.Position(8, 9),
			),
	...
	test("Completion Item Full", () => {
		...
		rangeEqual(
			result.additionalTextEdits![0].range,
			item.additionalTextEdits[0].range,
		);
		strictEqual(
			result.additionalTextEdits![0].newText,
	...
	test("Completion Item - insertText", () => {
		const item: ProtocolCompletionItem = new ProtocolCompletionItem(
			"label",
		);
		item.insertText = "insert";
		item.fromEdit = false;
	...
	test("Completion Item - TextEdit", () => {
		...
		item.textEdit = vscode.TextEdit.insert(
			new vscode.Position(1, 2),
			"insert",
		);
		item.fromEdit = false;
	...
	test("DiagnosticSeverity", () => {
		strictEqual(
			c2p.asDiagnosticSeverity(<any>vscode.DiagnosticSeverity.Error),
			proto.DiagnosticSeverity.Error,
		);
		strictEqual(
			c2p.asDiagnosticSeverity(<any>vscode.DiagnosticSeverity.Warning),
			proto.DiagnosticSeverity.Warning,
		);
		strictEqual(
			c2p.asDiagnosticSeverity(
				<any>vscode.DiagnosticSeverity.Information,
			),
			proto.DiagnosticSeverity.Information,
		);
		strictEqual(
			c2p.asDiagnosticSeverity(<any>vscode.DiagnosticSeverity.Hint),
			proto.DiagnosticSeverity.Hint,
		);
	});

	test("DiagnosticTag", () => {
		strictEqual(
			c2p.asDiagnosticTag(<any>vscode.DiagnosticTag.Unnecessary),
			proto.DiagnosticTag.Unnecessary,
		);
		strictEqual(
			c2p.asDiagnosticTag(<any>vscode.DiagnosticTag.Deprecated),
			proto.DiagnosticTag.Deprecated,
		);
	});

	test("Diagnostic", async () => {
		const item: vscode.Diagnostic = new vscode.Diagnostic(
		...
		item.relatedInformation = [
			new vscode.DiagnosticRelatedInformation(
				new vscode.Location(
					vscode.Uri.parse("file://localhost/folder/file"),
		...
		strictEqual(result.relatedInformation![0].message, "related");
		strictEqual(
			result.relatedInformation![0].location.uri,
			"file://localhost/folder/file",
		);
		strictEqual(
			result.relatedInformation![0].location.range.end.character,
		...
		ok(
			(await c2p.asDiagnostics([item])).every((elem) =>
				proto.Diagnostic.is(elem),
			),
		);
	});
	...
	test("Diagnostic - Complex Code", () => {
		...
		item.code = {
			value: 99,
			target: vscode.Uri.parse("https://code.visualstudio.com/"),
		};
		...
		strictEqual(
			result.codeDescription?.href,
			"https://code.visualstudio.com/",
		);
	});

	test("Diagnostic - Complex Code - Deprecated", () => {
		const item: ProtocolDiagnostic = new ProtocolDiagnostic(
			new vscode.Range(1, 2, 8, 9),
			"message",
			vscode.DiagnosticSeverity.Warning,
			undefined,
	...
	test("InlayHint", () => {
		const item: ProtocolInlayHint = new ProtocolInlayHint(
	...
	test("InlineCompletionContext", () => {
		...
		const result: proto.InlineCompletionContext =
			c2p.asInlineCompletionContext(item);

		strictEqual(
			result.triggerKind,
			proto.InlineCompletionTriggerKind.Invoked,
		);
		assertDefined(result.selectedCompletionInfo);
		rangeEqual(
			result.selectedCompletionInfo.range,
			item.selectedCompletionInfo!.range,
		);
		strictEqual(
			result.selectedCompletionInfo.text,
			item.selectedCompletionInfo!.text,
		);
	});
});
