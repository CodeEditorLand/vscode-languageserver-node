...
import type { _Notebooks, Connection, Feature } from "./server";
import {
...
export const NotebookSyncFeature: Feature<
	_Notebooks,
...
export class NotebookDocuments<T extends { uri: DocumentUri }> {
	...
	constructor(
		...
		if (configurationOrTextDocuments instanceof TextDocuments) {
			...
			this._cellTextDocuments = new TextDocuments<T>(
				configurationOrTextDocuments,
			);
		}
	...
	public findNotebookDocumentForCell(
					...
					if (change.cells !== undefined) {
						...
						if (changedCells.structure !== undefined) {
							...
							notebookDocument.cells.splice(
								array.start,
								array.deleteCount,
								...(array.cells !== undefined
									? array.cells
									: []),
							);
							// Additional open cell text documents.
							if (changedCells.structure.didOpen !== undefined) {
								for (const open of changedCells.structure
									.didOpen) {
									await cellTextDocumentConnection.openTextDocument(
										{ textDocument: open },
									);
							...
							if (changedCells.structure.didClose) {
								for (const close of changedCells.structure
									.didClose) {
