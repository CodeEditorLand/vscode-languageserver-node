...
import { DiagnosticFeature, DiagnosticFeatureShape } from "./diagnostic";
import {
	FileOperationsFeature,
	FileOperationsFeatureShape,
} from "./fileOperations";
import { FoldingRangeFeature, FoldingRangeFeatureShape } from "./foldingRange";
// import { InlineCompletionFeatureShape, InlineCompletionFeature } from './inlineCompletion.proposed';
import { InlayHintFeature, InlayHintFeatureShape } from "./inlayHint";
import { InlineValueFeature, InlineValueFeatureShape } from "./inlineValue";
import {
...
import { NotebookSyncFeature, NotebookSyncFeatureShape } from "./notebook";
import {
	attachPartialResult,
	attachWorkDone,
	ProgressFeature,
...
import { ShowDocumentFeature, ShowDocumentFeatureShape } from "./showDocument";
...
class RemoteConsoleImpl implements Logger, RemoteConsole, Remote {
	...
	private send(type: MessageType, message: string): void {
		if (this._rawConnection) {
			this._rawConnection
				.sendNotification(LogMessageNotification.type, {
					type,
					message,
				})
				.catch(() => {
...
export type RemoteWindow = _RemoteWindow &
	WindowProgress &
...
const RemoteWindowImpl: new () => RemoteWindow = ShowDocumentFeature(
	ProgressFeature(_RemoteWindowImpl),
) as new () => RemoteWindow;
...
class RemoteClientImpl implements RemoteClient, Remote {
	...
	public register(
		typeOrRegistrations:
			...
			| BulkRegistration
			| BulkUnregistration,
		registerOptionsOrType?: string | MethodType | any,
		registerOptions?: any,
		...
		if (typeOrRegistrations instanceof BulkRegistrationImpl) {
			...
			return this.registerSingle1(
				...
				<string | MethodType>registerOptionsOrType,
				registerOptions,
			);
		} else {
			return this.registerSingle2(
				<string | MethodType>typeOrRegistrations,
...
export type RemoteWorkspace = _RemoteWorkspace &
	Configuration &
	WorkspaceFolders &
	FileOperationsFeatureShape;
...
class _RemoteWorkspaceImpl implements _RemoteWorkspace, Remote {
	...
	public applyEdit(
		...
		const params: ApplyWorkspaceEditParams = isApplyWorkspaceEditParams(
			paramOrEdit,
		)
			? paramOrEdit
			: { edit: paramOrEdit };
...
const RemoteWorkspaceImpl: new () => RemoteWorkspace = FileOperationsFeature(
	WorkspaceFoldersFeature(ConfigurationFeature(_RemoteWorkspaceImpl)),
...
class TelemetryImpl implements Telemetry, Remote {
	...
	public logEvent(data: any): void {
		this.connection
			.sendNotification(TelemetryEventNotification.type, data)
			.catch(() => {
			...
			InlayHintFeature(
				InlineValueFeature(
					TypeHierarchyFeature(
						LinkedEditingRangeFeature(
							SemanticTokensFeature(
								CallHierarchyFeature(_LanguagesImpl),
							),
						),
					),
				),
...
> {
	...
	onWorkspaceSymbol(
		handler: ServerRequestHandler<
			WorkspaceSymbolParams,
			SymbolInformation[] | WorkspaceSymbol[] | undefined | null,
			SymbolInformation[],
			void
		>,
	): Disposable;
	...
	onWorkspaceSymbolResolve(
		handler: RequestHandler<WorkspaceSymbol, WorkspaceSymbol, void>,
	): Disposable;
	 ...
	 */
	onCodeActionResolve(
		handler: RequestHandler<CodeAction, CodeAction, void>,
	): Disposable;
	 ...
	 */
	onCodeLensResolve(
		handler: RequestHandler<CodeLens, CodeLens, void>,
	): Disposable;
...
export function combineConsoleFeatures<O, T>(
	...
	return function (
		Base: new () => RemoteConsole,
	): new () => RemoteConsole & O & T {
...
export function combineTracerFeatures<O, T>(
	...
	return function (
		Base: new () => RemoteTracer,
	): new () => RemoteTracer & O & T {
		return two(one(Base)) as any;
	};
}
...
export function combineClientFeatures<O, T>(
	...
	return function (
		Base: new () => RemoteClient,
	): new () => RemoteClient & O & T {
		return two(one(Base)) as any;
	};
}
...
export function combineWindowFeatures<O, T>(
	...
	return function (
		Base: new () => _RemoteWindow,
	): new () => _RemoteWindow & O & T {
		return two(one(Base)) as any;
	};
}
...
export function combineWorkspaceFeatures<O, T>(
	...
	return function (
		Base: new () => _RemoteWorkspace,
	): new () => _RemoteWorkspace & O & T {
		return two(one(Base)) as any;
	};
}
...
> {
	...
	> &
		...
		onRequest: <R, E>(
			type: string | MessageSignature | StarRequestHandler,
			handler?: GenericRequestHandler<R, E>,
		): Disposable => (connection as any).onRequest(type, handler),
		...
		onNotification: (
			type: string | MessageSignature | StarNotificationHandler,
			handler?: GenericNotificationHandler,
		): Disposable => (connection as any).onNotification(type, handler),
		...
		onWorkspaceSymbolResolve: (handler) =>
			connection.onRequest(WorkspaceSymbolResolveRequest.type, handler),
	...
	connection.onRequest<void, void, void, unknown>(
