	...
	void connection.workspace.getWorkspaceFolders().then((folders) => {
		for (const folder of folders) {
			connection.console.log(
				`Get workspace folders: ${folder.name} ${folder.uri}`,
			);
		}
	});
	...
	return [
		{
			...
			originSelectionRange: {
				start: {
					...
					line: params.position.line,
					character: params.position.line + 4,
				},
			},
		},
	];
...
connection.onCodeAction((params) => {
	...
	a.insert(
		{ line: 0, character: 0 },
		"Code Action",
		ChangeAnnotation.create("Insert some text", true),
	);

	const b = change.getTextEditChange({
	...
	b.insert(
		{ line: 0, character: 0 },
		"The initial content",
		ChangeAnnotation.create("Add additional content", true),
	);
...
connection.onDocumentRangeFormatting((params) => {
	connection.console.log(
		`Document Range Formatting: ${JSON.stringify(params.range)} ${JSON.stringify(params.options)}`,
	);
...
connection.onDocumentOnTypeFormatting((params) => {
	connection.console.log(
		`Document On Type Formatting: ${JSON.stringify(params.position)} ${params.ch} ${JSON.stringify(params.options)}`,
	);
...
connection.onRenameRequest((params) => {
	connection.console.log(
		`Rename: ${JSON.stringify(params.position)} ${params.newName}`,
	);
...
const not: ProtocolNotificationType<string[], void> =
	new ProtocolNotificationType<string[], void>("testbed/notification");
