...
const _ril: RIL = Object.freeze<RIL>({
			...
			encode: (
				...
				try {
					return Promise.resolve(
						Buffer.from(
							JSON.stringify(msg, undefined, 0),
							options.charset,
						),
					);
				} catch (err) {
			...
			decode: (
				...
				try {
					if (buffer instanceof Buffer) {
						...
						return Promise.resolve(
							JSON.parse(
								new TextDecoder(options.charset).decode(buffer),
							),
						);
					}
				} catch (err) {
	...
	stream: Object.freeze({
		asReadableStream: (stream: NodeJS.ReadableStream) =>
			new ReadableStreamWrapper(stream),
		asWritableStream: (stream: NodeJS.WritableStream) =>
			new WritableStreamWrapper(stream),
	}),
	console: console,
	timer: Object.freeze({
		setTimeout(
